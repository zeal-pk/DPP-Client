...
export default function BackupAddProducts({ params }) {
  ...
  const [openSubTab, setOpenSubTab] = React.useState(false);
  const handleOpenSubTab = () => setOpenSubTab(true);
  const handleCloseSubTab = () => {
    setOpenSubTab(false);
    setSubTabArray([]);
  };
  const [openFieldSelection, setOpenFieldSelection] = React.useState(false);
  ...
  const [openAddChart, setOpenAddChart] = React.useState(false);
        ...
        <Fade in={openAddChart}>
          <Box sx={style}>
            <div>
              ...
              <TextField onChange={(e) => setYAxis(e.target.value)} />
              <Button
                onClick={() => {
                  let chartLable = {
                    chartType: chartType,
            ...
            <Button onClick={handleCloseAddChart}>Save</Button>
          </Box>
        </Fade>
      </Modal>
    </div>
